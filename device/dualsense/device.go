...
func new(o *device.CreateOptions, edge bool) (*DualSense, error) {
	...
	d.inputConnection.Store(uint32(metaState.ConnectionStatus))
	d.input = newDualSenseInputScheduler(metaState.BatteryStatus,
		metaState.ConnectionStatus, edge)
	if edge {
...
func (d *DualSense) SetMetaState(meta MetaState) {
	...
	d.inputConnection.Store(uint32(meta.ConnectionStatus))
}
...
func (d *DualSense) UpdateMetaState(meta MetaState) {
	...
	d.metaState = &current
	d.inputConnection.Store(uint32(current.ConnectionStatus))
}
...
func (d *DualSense) ClaimInputReport(destination []byte) (int, uint64) {
	...
	connection := byte(d.inputConnection.Load())
	d.input.mu.Lock()
	...
	now := time.Now()
	n, token := d.input.beginClaim(now, battery, connection, destination)
	d.input.mu.Unlock()
	return n, token
}
...
func (d *DualSense) buildUSBInputReport(s *InputState, m *MetaState) []byte {
	...
	battery := byte(0)
	connection := byte(dualSenseUSBConnectStateWired)
	if m != nil {
	...
	if !encodeUSBInputReportInto(s, battery, connection, 1, 1, timestamp,
		d.input.edge, b) {
		d.input.mu.Lock()
...
func resetUSBInputReportToNeutral(b []byte, seq uint8, packetSequence,
	timestamp uint32, battery, connection byte, edge bool, metadata *InputState) {
	for i := range b {
		b[i] = 0
	}
	...
	encodeUSBInputMetadata(b, metadata, timestamp, edge, battery, connection)
}
