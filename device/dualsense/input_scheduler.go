...
func newDualSenseInputScheduler(battery, connection byte,
	edge bool) *dualSenseInputScheduler {
	now := time.Now()
	...
	encodeUSBInputReportInto(&neutral, battery, connection, 0, 0, 0, edge,
		s.lastReport[:])
	return s
}
...
func (s *dualSenseInputScheduler) beginClaim(now time.Time, battery,
	connection byte, destination []byte) (int, uint64) {
	...
	if !encodeUSBInputReportInto(&selected.state, battery, connection,
		sequence, packetSequence, timestamp, s.edge, s.claimedReport[:]) {
		s.corruptReports++
	}
	s.nextClaimToken++
...
func encodeUSBInputReportInto(state *InputState, battery, connection,
	sequence uint8, packetSequence, timestamp uint32, edge bool,
	destination []byte) bool {
...
func encodeUSBInputMetadata(report []byte, state *InputState, timestamp uint32,
	edge bool, battery, connection byte) {
